...
public class ReadExcel {
	...
	public static <T> List<T> getDataByExcel(String file, String[] fieldNames,
			String[] fieldTypes, Class<T> clazz, String exceptionFileName, HttpServletRequest request)
			throws IOException {
		...
		try {
			...
			if(clos != fieldNames.length){
				throw new RuntimeException("導入失敗，請選擇正確的模板");
			}
			if(rows>=2000){
				throw new RuntimeException("導入失敗，模板每次允许导入2000人");
			}
			...
			String configPath = HttpsUtil.getConfig("exception_inf_path");
			
			String path = (request.getSession().getServletContext().getRealPath(configPath) + exceptionFileName)
					.replace("/", File.separator);
			//文件夹位置不存在则创建
			String folderpath = request.getSession().getServletContext().getRealPath(configPath).replace("/", File.separator);
			File floderfile = new File(folderpath);
