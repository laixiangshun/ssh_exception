...
public class UploadFile {
	 ...
	 * @param request
	 * @param response
	 * @subdirectory 上传文件保存在服务器对应子目录名称
	 * @return 上传后保存的路径
	 * @throws IOException
	 */
	public static String uploadFile(HttpServletRequest request,HttpServletResponse response,String subdirectory) throws IOException{
		String path="";
		try {
		           ...
		           //获取multiRequest 中所有的文件名
		            ...
		            while(iter.hasNext())
		            {
		                ...
		                if(file!=null)
		                {
		                	...
		                	//根据配置文件创建指定目录
		                	String filepath=(HttpsUtil.getConfig("upload")+subdirectory+"/").replace("/", File.separator);
		                	String absolutePath=request.getSession().getServletContext().getRealPath(filepath);
		                	//File filesDirectory=new File((HttpsUtil.getConfig("courseFilePath")+subdirectory+"/").replace("/", File.separator));
		                	File filesDirectory=new File(absolutePath);
		                	if(!filesDirectory.exists())
		                	{
		                		filesDirectory.mkdirs();//创建多层目录
		                	}
		                	//将文件名转换为unique编码
		                	...
		                	//path=(HttpsUtil.getConfig("courseFilePath")+fileNameEcode+"."+rule).replace("/", File.separator);
		                	path=(filesDirectory+"/").replace("/", File.separator)+fileNameEcode+new Date().getTime()+"."+rule;
		                	 //上传文件
